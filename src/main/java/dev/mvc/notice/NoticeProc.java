...
public class NoticeProc implements NoticeProcInter {
    ...
    public List<NoticeVO> list_by_noticeno_search_paging(HashMap<String, Object> map) {
      ...
      */
      ...
      List<NoticeVO> list = this.noticeDAO.list_by_noticeno_search_paging(map);
      ...
      for (NoticeVO noticeVO : list) { // 내용이 160자 이상이면 160자만 선택
        ...
        String title = Tool.convertChar(noticeVO.getTitle());  // 특수 문자 변환
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
      ...
      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
        str.append("<span class='span_box_1'><A href='"+Contents.LIST2_FILE+"?&word="+word+"&now_page="+_now_page+"&noticeno="+noticeno+"'>이전</A></span>"); 
      } 
      ...
      for(int i=start_page; i<=end_page; i++){ 
        ...
        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='"+Contents.LIST2_FILE+"?word="+word+"&now_page="+i+"&noticeno="+noticeno+"'>"+i+"</A></span>");   
        } 
      } 
      ...
      if (now_grp < total_grp){ 
        str.append("<span class='span_box_1'><A href='"+Contents.LIST2_FILE+"?&word="+word+"&now_page="+_now_page+"&noticeno="+noticeno+"'>다음</A></span>"); 
      } 
      str.append("</DIV>"); 
