...
//  private MemberProcInter memberProc = null;
    ...
    public ModelAndView create(HttpSession session, Order_payVO order_payVO) { // order_payVO 자동 생성, Form -> VO
        ...
        /*
         * // 주문 결재하고 바로 번호 수집 <!-- 주문 결재 등록 전 order_payno를 Order_payVO에 저장 --> <insert
         * id="create" parameterType="dev.mvc.order_pay.Order_payVO"> <selectKey
         * keyProperty="order_payno" resultType="int" order="BEFORE"> SELECT
         * order_pay_seq.nextval FROM dual </selectKey>
         * 
         * INSERT INTO order_pay(order_payno, memberno, rname, rtel, rzipcode,
         * raddress1, raddress2, paytype, amount, rdate) VALUES (#{order_payno},
         * #{memberno}, #{rname}, #{rtel}, #{rzipcode}, #{raddress1}, #{raddress2},
         * #{paytype}, #{amount}, sysdate) </insert>
         */
        ...
        if (cnt == 1) { // 정상적으로 주문 결재 정보가 등록된 경우
            ...
            for (CartVO cartVO : list) {
                ...
                int tot = contentsVO.getPrice() * cartVO.getCnt(); // 할인 금액 합계 = 할인 금액 * 수량
