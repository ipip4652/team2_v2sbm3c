...
-- rownum: oralce system에서 select시에 자동으로 붙여주는 일련번호, 테이블에는 존재하지 않음.
...
-- FROM → SELECT → ROWNUM 생성 → ORDER BY ~
...
-- 6) 서브쿼리에서 SELECT 한 컬러만 외부 쿼리에서 사용 가능
SELECT no, title, rownum
FROM (
           SELECT no  
           FROM pg
