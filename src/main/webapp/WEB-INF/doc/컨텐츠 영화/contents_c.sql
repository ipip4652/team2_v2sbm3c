...
UPDATE contents
SET price = 10000, dc=10, saleprice=9000,  point=1000, salecnt=200
WHERE contentsno = 8;
...
SELECT contentsno, adminno, cateno, title, content, recom, cnt, replycnt, rdate,
           file1, file1saved, thumb1, size1, price, dc, saleprice, point
FROM contents
WHERE cateno=1 AND (title LIKE '%영화%' OR content LIKE '%영화%' OR word LIKE '%영화%')
ORDER BY contentsno DESC;
